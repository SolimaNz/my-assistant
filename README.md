...
By passing GPS coordinates (`lat`/`lng`) from the frontend client, Soli automatically calculates distances and customizes recommendations (e.g. nearby local food carts, pharmacies, ATMs, and transport options) specific to that exact spot.

### 3. Dynamic Live Web Search Fallback
To prevent stale knowledge, Soli uses a heuristic search classifier. Instead of making slow, costly searches for every greeting or simple logic check, it only triggers a live DuckDuckGo web search when it detects time-sensitive keywords:
* Weather conditions and current forecasts.
* Real-time currency exchange rates (EGP to USD/EUR).
* Public transport delays, event cancellations, or breaking news.
* Days of the week (both English and transliterated Arabic like `gom3a`, `sabt`).
* Islamic prayer times and religious holidays (Fajr, Dhuhr, Asr, Maghrib, Isha, Ramadan, Eid).
...
+-------+------------------+                    +---------+------------------+
        ...
        |-- 3. Trigger Web Search Heuristics              |-- 3. Stream Bi-directional 16kHz PCM
